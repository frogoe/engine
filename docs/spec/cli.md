...
**Status: LIVE (init/add/run/lint/check/bundle/embed/vision — check is the full gate: static + live sandbox).**
...
| `check`       | contract lint + authored-art contract (`art/*`) + output measures (contrast vs BRIEF palette, verb-vs-handlers, canvas painted, zero runtime requests) | `--json` [{code, file, line, severity, fix, recipe}] |
| `report`      | last playtest session on one screen: duration, fps mean, dips below 30 with wall-clock moments, errors, hidden periods | —                                                    |
| `bundle`      | externals dissolve → dist/index.html (see bundler spec) + authored art rasterizes → dist/assets/{poster,icon}.png    | `--json` { artifact, bytes, hashes, art }            |
| `embed`       | after check+bundle: the card dist/embed.html (poster loading state + sandboxed game) + dist/manifest.json            | —                                                    |
| `vision`      | the agent's eyes: SPRITES objects, gameplay frames, identity art as palette-aware ASCII maps + density metrics       | —                                                    |
