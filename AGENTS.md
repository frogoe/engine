...
frogoe export desktop|ios|android  # native app project in export/ (Tauri shell; appId defaults to com.frogoe.*)
```
...
- **game.test.js is the semantic gate** — logic verbs (type/swap/place) and round sessions ship headless tests via `bootForTest` from `"frogoe"` (pure exports + property-based + closure behavior with agent eyes `game.view()`); `frogoe check` runs them (`test/failed`, `test/logic-untested`)
- **`frogoe play` gives agents eyes + hands** — JSONL frames out (ground truth + ASCII map + `live()` coordinates when the game exports them), contract input in; step mode freezes the world (fast games become chess), `--record` keeps session evidence; `frogoe recap` turns evidence into machine facts + verdict skeleton (the AI annotates anomalies)
- **Evidence-gated claims** — a behavior claim ("agents can play it", "it works") ships with a recorded session artifact in the same commit; `frame.world` (engine-measured) + `frame.live` (game-declared) are the two coordinate channels
