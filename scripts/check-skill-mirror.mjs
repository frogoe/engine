...
// Ported from hyperframes scripts/check-skill-mirror.mjs:80
// Each CLI reads only its own path — drift means one agent ships stale skill.
...
const aExists = !!statSync(A, { throwIfNoEntry: false })?.isDirectory();
const bExists = !!statSync(B, { throwIfNoEntry: false })?.isDirectory();

if (!aExists && !bExists) {
  console.log(
    "Skill mirror check skipped: neither .claude/skills nor .agents/skills exists (run after Fase 5).",
  );
  process.exit(0);
}
if (!aExists || !bExists) {
  console.log(
    `Skill mirror OK (single mirror): ${aExists ? ".claude/skills" : ".agents/skills"} present, other not yet created — skip byte-identical check until Fase 5.`,
  );
  process.exit(0);
}
