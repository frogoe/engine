...
frogoe play                 # agent eyes+hands: frames out (ground truth + ASCII + live() coordinates), input in
frogoe recap [dir]          # session evidence → machine facts + verdict skeleton (the AI annotates anomalies)
...
3. **Gameplay uses `stage.play`** (capped centered column: `left/right/center/width`), never raw `window.innerWidth` — identical challenge on every screen width. **Stage geometry is live** (free-size desktop windows, the iOS keyboard): re-read it per tick, never destructure it into a const; when an anchor moves (floor, arena center) CARRY entities by the delta or store normalized coords remapped on change; center overlays with `inset: 0` + grid, never tuned `%`.
