/** frogoe check — deterministic contract lint for a game folder.
 *  Findings carry stable codes, exact locations, fix shapes, and recipe
 *  pointers so agents self-heal in one iteration. Exit 1 on any error. */
...
/** WCAG relative luminance + contrast ratio from two hex colors. */
...
const checkBrief = (dir: string, findings: Finding[]): void => {
  ...
  for (const key of ["bg", "fg", "accent"] as const) {
    if (!brief[key] || !HEX.test(brief[key] ?? "")) {
