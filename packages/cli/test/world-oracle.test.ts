...
const collect = async (name: string, frames: number) => {
  ...
  for (let i = 0; i < frames; i += 1) {
    ...
    const hinted =
      f.world?.playerHint !== null && f.world?.playerHint !== undefined
        ? f.world.entities[f.world.playerHint]!
        : null;
    // a blob (many shapes merged under load — actor+walls fused) is not
    // the actor; comparing against it would fail for honest reasons
    const world = hinted !== null && hinted.w <= 120 ? { x: hinted.x, y: hinted.y } : null;
...
describe("world oracle (real browser, real game)", () => {
  test("flappy: engine-measured bird agrees with game-declared live()", async () => {
  ...
  test("world absent is legal: games without live() still measure", async () => {
